@if (auth()->user()->can('master_data_edit') || auth()->user()->can('master_data_delete'))
    <div class="d-flex justify-content-end">
        @can('master_data_show')
            <a href="{{ route('master-data.organization-units.show', $model->organization_id) }}"
                class="btn btn-sm btn-primary btn-icon" type="button" >View
            </a>
        @endcan
        @can('master_data_edit')
            <a href="{{ route('master-data.organization-units.edit', $model->organization_id) }}"
                class="btn btn-sm btn-primary btn-icon" type="button" >Edit
            </a>
        @endcan
        @can('master_data_delete')
            <form action="{{ route('master-data.organization-units.destroy', $model->organization_id) }}"
                method="POST">
                @csrf
                @method('DELETE')
